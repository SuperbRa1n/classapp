  ...
  methods: {
    async initializeData() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/grade/grade.vue:139", "Error initializing data:", error);
      }
    },
  ...
  return {
    ...
    b: common_vendor.f($data.termSwitcher, (term, index, i0) => {
      return {
        ...
        b: common_vendor.t($options.calculateXueqiGPA(term).toFixed(2)),
        c: common_vendor.t($options.calculateXueqiCredit(term).toFixed(2)),
        d: index,
        e: common_vendor.n({
          "selected-term-item": term === $data.selectedTerm
        }),
        f: common_vendor.o(($event) => $options.selectTerm(term), index)
      };
    }),
